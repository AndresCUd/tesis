...
void esclavo(void)
{
        ...
        if ( e == 0 )
        {
            ...
            if (my_packet[0] == 97 ){
              ...
              // Esto de LoRa
              sprintf(info1,"%d%s%d%s%d%s%f%s%d%s%d%s%d%s%d%s%d",sx1272._nodeAddress,coma,paqueteEnviado,coma,paqueterecibido,coma,secs,coma,sx1272._RSSI,coma,sx1272._bandwidth,coma,sx1272._maxCurrent,coma,sx1272._payloadlength);
              // Estado GNSS + Datos
